...
impl VehicleState {
    ...
    pub fn compare(&self, new: &VehicleState, force: bool) -> Vec<u8> {
        let verbose = false;
        let mut buffer: Vec<u8> = vec![0; 0];

        if (self.ignition != new.ignition) || force {
            if verbose {
                println!("ignition: {} -> {} ", self.ignition, new.ignition);
            }
        ...
        if (self.engine != new.engine) || force {
            if verbose {
                println!("engine: {} -> {} ", self.engine, new.engine);
            }
        ...
        if (self.doors != new.doors) || force {
            if verbose {
                println!("doors: {} -> {} ", self.doors, new.doors);
            }
        ...
        if (self.indicator != new.indicator) || force {
            if verbose {
                println!("indicator: {} -> {} ", self.indicator, new.indicator);
            }
        ...
        if (self.lights_main != new.lights_main) || force {
            if verbose {
                println!("lights_main: {} -> {} ", self.lights_main, new.lights_main);
            }
        ...
        if (self.fuel != new.fuel) || force {
            if verbose {
                println!("fuel:  {} -> {} ", self.fuel, new.fuel);
            }
        ...
        if (self.speed != new.speed) || force {
            if verbose {
                println!("speed:  {} -> {} ", self.speed, new.speed);
            }
        ...
        if (self.maxspeed != new.maxspeed) || force {
            if verbose {
                println!("maxspeed:  {} -> {} ", self.maxspeed, new.maxspeed);
            }
